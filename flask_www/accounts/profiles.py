...
@profiles_bp.route('/profile/update/ajax', methods=['GET', 'POST'])
@login_required
def update_ajax():
...
def vendor_detail(_id):
    ...
    return render_template('accounts/profiles/profile_detail.html', profile=profile_obj, cover_img=cover_img_obj)
...
@profiles_bp.route('/profile/vendor/update/ajax', methods=['POST'])
@login_required
def vendor_update_ajax():
    user_id = current_user.id
    ...
    if request.method == 'POST':
        if profile:
            ...
            return make_response(jsonify(data_response))
            # return redirect(url_for('profiles.vendor_detail', _id=profile.id))
        else:
            abort(401)


@profiles_bp.route('/profile/vendor/update/email/send/ajax', methods=['POST'])
@login_required
def vendor_update_email_send_ajax():
    subject = "판매사업자 신청 메일"
    user_email = current_user.email
    token = None
    msg_txt = 'accounts/send_mails/account_update_register_mail.txt'
...
@profiles_bp.route('/profile_cover_images/save/ajax', methods=['POST'])
@login_required
def account_cover_img_save_ajax():
    ajax_post_key()
...
@profiles_bp.route('/profile_cover_images/delete/ajax', methods=['POST'])
@login_required
def account_cover_img_delete_ajax():
    user_id = current_user.id
    profile = db.session.query(Profile).filter_by(user_id=user_id).first()
    existing_cover_img = db.session.query(ProfileCoverImage).filter_by(profile_id=profile.id).first()
    if request.method == 'POST':
